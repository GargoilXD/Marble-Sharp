...
public class Parser {
    ...
    private Node get_operand_node() {
        switch (CurrentToken) {
            ...
            case DataToken data_token:
                if (data_token.Type == DataToken.TYPE.WORD && (Index + 1 < Tokens.Count) && Tokens[Index + 1] is DataToken){
                    next_token();
    ...
    private Node deal_with_modifier_structure() {
        switch (CurrentToken) {
            case KeywordToken keyword_token:
                switch (keyword_token.Type) {
                    case KeywordToken.TYPE.MODIFIER:
                        next_token();
                        return new UnaryOperatorNode(keyword_token, deal_with_modifier_structure());
                    case KeywordToken.TYPE.DATATYPE:
    ...
    private Node get_variable_structure() {
        ...
        return new UnaryOperatorNode(datatype, get_binary_node(get_operand_node, new [] {OperatorToken.OPERATOR.ASSIGN}, () => get_expression()));
    }
    private Node get_function_structure(KeywordToken keyword_token) {
        ...
        InstructionListNode instruction = get_instruction_list_node();
            ...
            switch (CurrentToken) {
                case KeywordToken keyword_token:
                    switch (keyword_token.Keyword) {
                        ...
                        case KeywordToken.KEYWORD.CONSTANT: case KeywordToken.KEYWORD.STATIC:
                        case KeywordToken.KEYWORD.PUBLIC: case KeywordToken.KEYWORD.PRIVATE:

                        case KeywordToken.KEYWORD.FUNCTION: case KeywordToken.KEYWORD.CLASS: case KeywordToken.KEYWORD.CONSTRUCTOR:

                        case KeywordToken.KEYWORD.VOID: case KeywordToken.KEYWORD.VARIANT:
                        case KeywordToken.KEYWORD.BOOLEAN:case KeywordToken.KEYWORD.INTEGER:
                        case KeywordToken.KEYWORD.FLOAT: case KeywordToken.KEYWORD.STRING:
                        case KeywordToken.KEYWORD.LIST: case KeywordToken.KEYWORD.DICTIONARY:
                            nodes.Add(deal_with_modifier_structure());
                            break;
                        case KeywordToken.KEYWORD.ELSE_IF: case KeywordToken.KEYWORD.ELSE:
